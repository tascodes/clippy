<script lang="ts">
	...
	const noMatches = $derived(term.length >= 1 && matchCount === 0 && !invalidRegex);
	const hasError = $derived(noMatches || invalidRegex);
</script>

<div class="flex items-center gap-1.5 border-b-2 border-black bg-white px-3 py-2">
	<!-- Search input -->
	<div
		class="flex flex-1 items-center gap-1 border-2 px-2 py-1 transition-colors
	...
	>
		...
		<div class="flex items-center gap-0.5 border-l border-gray-200 pl-1">
			<button
				...
				title="Match case"
				class="rounded px-1 py-0.5 font-sans text-xs font-bold transition-colors
					{caseSensitive
			...
			<button
				...
				class="rounded px-1 py-0.5 font-mono text-xs font-bold transition-colors
					{regexEnabled
					? 'bg-purple-100 text-purple-700'
					: 'text-gray-400 hover:bg-gray-100 hover:text-gray-600'}"
			>.*</button>
		...
		{#if term.length > 0}
			<button onclick={() => oninput('')} class="text-gray-400 hover:text-gray-600" aria-label="Clear">
	...
	<!-- Match counter -->
	<span class="min-w-[80px] text-center font-mono text-xs tabular-nums text-gray-500">
		{#if term.length === 0}
			&nbsp;
		{:else if invalidRegex}
			<span class="font-semibold text-red-500">Invalid regex</span>
		{:else if noMatches}
